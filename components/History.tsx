    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      {viewingBatch && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <div className={`relative bg-white w-full max-w-2xl shadow-2xl border-t-8 border-emerald-600 animate-in zoom-in duration-200 overflow-hidden`}>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
              <button onClick={() => window.print()} className={`flex-1 flex items-center justify-center gap-2 py-3 bg-emerald-600 text-white text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-black/10 hover:bg-emerald-700 transition-all`}>
                <Printer className="w-3.5 h-3.5" /> Imprimir
              </button>
              <button onClick={() => setViewingBatch(null)} className="flex-1 py-3 bg-white border border-slate-200 text-slate-400 text-[10px] font-bold uppercase tracking-widest hover:bg-slate-100 transition-all">
                Fechar
              </button>
            </div>
          </div>
      ...
      <div className="flex border-b border-slate-200">
        <button 
          ...
          className={`px-6 py-4 text-[10px] font-black uppercase tracking-widest transition-all border-b-2 ${activeTab === 'outflows' ? `${theme.border} ${theme.text}` : 'border-transparent text-slate-400 hover:text-slate-600'}`}
        >
        ...
        <button 
          ...
          className={`px-6 py-4 text-[10px] font-black uppercase tracking-widest transition-all border-b-2 ${activeTab === 'entries' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
        >
          <div className="flex items-center gap-2">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="px-6 py-4 text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em]">Resumo do Lote</th>
                    <th className="px-6 py-4 text-center text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em]">Itens</th>
                    <th className="px-6 py-4 text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em]">Operador</th>
                    <th className="px-6 py-4 text-center text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em]">Documento</th>
                    <th className="px-6 py-4 text-right text-[9px] font-bold text-slate-400 uppercase tracking-[0.3em]">Remover</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {groupedEntries.length > 0 ? (
                    groupedEntries.map((batch, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-center">
                            <button onClick={() => setViewingBatch(batch)} className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-all text-[9px] font-bold uppercase tracking-widest">
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-20 text-center text-[10px] uppercase font-bold tracking-[0.3em] text-slate-300">Nenhum lote localizado</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            {groupedEntries.map((batch, idx) => {
              ...
              return (
                <div key={idx} className="bg-white border border-slate-200 shadow-sm relative overflow-hidden group">
                  ...
                  <button onClick={() => setViewingBatch(batch)} className="w-full py-4 bg-emerald-50 text-emerald-600 flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest border-t border-emerald-100 active:bg-emerald-100 transition-all">
