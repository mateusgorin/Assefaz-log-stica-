  ...
  return (
    <div className="space-y-6 sm:space-y-8">
          ...
          <div className={`relative bg-white w-full max-w-2xl shadow-2xl border-t-8 ${theme.border} animate-in zoom-in duration-200 overflow-hidden`}>
            <div className="p-6 sm:p-8 border-b border-slate-100 flex justify-between items-start">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-[16px] font-semibold text-slate-800 uppercase tracking-tighter">Comprovante de Saída</h3>
                  <p className="text-[11px] text-slate-400 font-semibold uppercase tracking-widest">ID: {viewingMovementBatch[0].batchId || viewingMovementBatch[0].id}</p>
                </div>
              </div>
            ...
            <div className="p-6 sm:p-8 space-y-8">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                <div>
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1">Setor Solicitante</p>
                  <p className="text-[12px] font-semibold text-slate-700 uppercase">{getSector(viewingMovementBatch[0].sectorId)?.name || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1">Unidade / Data</p>
                  <p className="text-[12px] font-semibold text-slate-700 uppercase">{viewingMovementBatch[0].unit.toUpperCase()}</p>
                  <p className="text-[11px] text-slate-400 uppercase font-normal">{viewingMovementBatch[0].date} às {viewingMovementBatch[0].time}</p>
                </div>
                <div className="text-right">
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1">Responsável</p>
                  <p className="text-[12px] font-semibold text-slate-700 uppercase">{getStaff(viewingMovementBatch[0].stockStaffId)?.name || 'N/A'}</p>
                </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      <th className="px-4 py-2 text-[11px] font-semibold text-slate-400 uppercase tracking-widest">Item Retirado</th>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {viewingMovementBatch.map((item, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="px-4 py-3 text-[12px] font-bold text-slate-900 text-right">{item.quantity} {getProduct(item.productId)?.unit || 'UN'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
              <button onClick={() => window.print()} className={`flex-1 flex items-center justify-center gap-2 py-3 ${theme.badge} text-white text-[12px] font-semibold uppercase tracking-widest shadow-lg shadow-black/10 hover:opacity-90 transition-all`}>
                <Printer className="w-3.5 h-3.5" /> Imprimir
              </button>
              <button onClick={() => setViewingMovementBatch(null)} className="flex-1 py-3 bg-white border border-slate-200 text-slate-400 text-[12px] font-semibold uppercase tracking-widest hover:bg-slate-100 transition-all">
                Fechar
              </button>
            </div>
          </div>
          ...
          <div className={`relative bg-white w-full max-w-2xl shadow-2xl border-t-8 border-emerald-600 animate-in zoom-in duration-200 overflow-hidden`}>
            <div className="p-6 sm:p-8 border-b border-slate-100 flex justify-between items-start">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-[16px] font-semibold text-slate-800 uppercase tracking-tighter">Ficha de Recebimento</h3>
                  <p className="text-[11px] text-slate-400 font-semibold uppercase tracking-widest">Lote: {viewingBatch[0].batchId || viewingBatch[0].id}</p>
                </div>
              </div>
            ...
            <div className="p-6 sm:p-8 space-y-8">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1">Operador Responsável</p>
                  <p className="text-[12px] font-semibold text-slate-700 uppercase">{getStaff(viewingBatch[0].stockStaffId)?.name || 'N/A'}</p>
                </div>
                <div className="text-right">
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest mb-1">Unidade / Data / Hora</p>
                  <p className="text-[12px] font-semibold text-slate-700 uppercase">{viewingBatch[0].unit.toUpperCase()}</p>
                  <p className="text-[11px] text-slate-400 uppercase font-normal">{viewingBatch[0].date} às {viewingBatch[0].time}</p>
                </div>
              </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
              <button onClick={() => window.print()} className={`flex-1 flex items-center justify-center gap-2 py-3 bg-emerald-600 text-white text-[12px] font-semibold uppercase tracking-widest shadow-lg shadow-black/10 hover:bg-emerald-700 transition-all`}>
                <Printer className="w-3.5 h-3.5" /> Imprimir
              </button>
              <button onClick={() => setViewingBatch(null)} className="flex-1 py-3 bg-white border border-slate-200 text-slate-400 text-[12px] font-semibold uppercase tracking-widest hover:bg-slate-100 transition-all">
                Fechar
              </button>
            </div>
          </div>
      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          <h1 className="text-[20px] font-semibold text-[#14213D] uppercase tracking-tighter">Histórico Oficial</h1>
          <p className="text-[12px] text-slate-500 mt-1 uppercase tracking-[0.2em] font-normal">Registro Cronológico de Movimentações</p>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input 
              ...
              className={`pl-9 pr-4 py-2.5 bg-white border border-slate-200 text-[12px] font-normal uppercase tracking-[0.2em] outline-none ${theme.primaryFocus} w-full sm:w-64 shadow-sm`}
            />
          </div>
        </div>
      </header>
      ...
      <div className="flex border-b border-slate-200">
        <button 
          ...
          className={`px-6 py-4 text-[12px] font-semibold uppercase tracking-widest transition-all border-b-2 ${activeTab === 'outflows' ? `${theme.border} ${theme.text}` : 'border-transparent text-slate-400 hover:text-slate-600'}`}
        >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="px-6 py-4 text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Data/Hora</th>
                    <th className="px-6 py-4 text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Setor</th>
                    <th className="px-6 py-4 text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Resumo do Lote</th>
                    <th className="px-6 py-4 text-center text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Itens</th>
                    <th className="px-6 py-4 text-center text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Documento</th>
                    <th className="px-6 py-4 text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Responsável</th>
                    <th className="px-6 py-4 text-right text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Remover</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {groupedMovements.length > 0 ? (
                      ...
                      return (
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <p className="text-[12px] font-semibold text-slate-700">{batch[0].date}</p>
                            <p className="text-[11px] text-slate-400 tracking-tighter">{batch[0].time}</p>
                          </td>
                          <td className="px-6 py-4 text-[12px] font-semibold text-slate-600 uppercase">
                            {sector?.name || 'Setor'}
                          </td>
                          <td className="px-6 py-4 text-[12px] text-slate-700 font-normal uppercase">
                            {firstProduct?.name || 'Item'} {batch.length > 1 ? `e mais ${batch.length - 1}...` : ''}
                          </td>
                          <td className="px-6 py-4 text-center">
                            <span className="inline-flex items-center px-2 py-1 bg-slate-100 text-slate-700 text-[12px] font-bold rounded">
                              {batch.length}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-center">
                            <button onClick={() => setViewingMovementBatch(batch)} className={`inline-flex items-center gap-2 px-3 py-1.5 ${theme.light} ${theme.text} hover:bg-slate-200 transition-all text-[12px] font-semibold uppercase tracking-widest`}>
                              <Eye className="w-3.5 h-3.5" /> Ver DOC
                            </button>
                          </td>
                          <td className="px-6 py-4 text-[12px] text-slate-500 uppercase">
                    ...
                    <tr>
                      <td colSpan={7} className="px-6 py-20 text-center text-[12px] uppercase font-semibold tracking-[0.3em] text-slate-300">Nenhum registro localizado</td>
                    </tr>
                  )}
                </tbody>
              </table>
                ...
                <div key={idx} className="bg-white border border-slate-200 shadow-sm relative overflow-hidden group">
                  ...
                  <div className="p-5 pb-0">
                    <div className="flex items-center gap-3 border-b border-slate-50 pb-3 mb-4">
                      ...
                      <div className="pr-12">
                        <p className="text-[12px] font-semibold text-slate-800 uppercase leading-none mb-1 truncate">
                          {firstProduct?.name || 'Item'} {batch.length > 1 ? `+${batch.length - 1}` : ''}
                        </p>
                        <p className="text-[11px] text-slate-400 uppercase tracking-tighter font-semibold">Comprovante com {batch.length} itens</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-[12px] font-semibold text-slate-600 uppercase truncate">{sector?.name || 'Setor'}</p>
                      </div>
                      <div className="space-y-1 text-right">
                        <div className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-400 uppercase tracking-wider justify-end">
                          <Clock className="w-2.5 h-2.5" /> Horário
                        </div>
                        <p className="text-[12px] font-semibold text-slate-600 uppercase">{batch[0].date} - {batch[0].time}</p>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => setViewingMovementBatch(batch)} className={`w-full py-4 ${theme.light} ${theme.text} flex items-center justify-center gap-3 text-[12px] font-semibold uppercase tracking-widest border-t border-slate-100 active:bg-slate-200 transition-all`}>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="px-6 py-4 text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Resumo do Lote</th>
                    <th className="px-6 py-4 text-center text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Itens</th>
                    <th className="px-6 py-4 text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Operador</th>
                    <th className="px-6 py-4 text-center text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Documento</th>
                    <th className="px-6 py-4 text-right text-[11px] font-semibold text-slate-400 uppercase tracking-[0.3em]">Remover</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {groupedEntries.length > 0 ? (
                    groupedEntries.map((batch, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-center">
                            <span className="inline-flex items-center px-2 py-1 bg-emerald-50 text-emerald-700 text-[12px] font-bold rounded">
                              {batch.length}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-[12px] text-slate-500 uppercase">
                          ...
                          <td className="px-6 py-4 text-center">
                            <button onClick={() => setViewingBatch(batch)} className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-all text-[12px] font-semibold uppercase tracking-widest">
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-20 text-center text-[12px] uppercase font-semibold tracking-[0.3em] text-slate-300">Nenhum lote localizado</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            {groupedEntries.map((batch, idx) => {
              ...
              return (
                <div key={idx} className="bg-white border border-slate-200 shadow-sm relative overflow-hidden group">
                  ...
                  <div className="p-5 pb-0">
                    <div className="flex items-center gap-3 border-b border-slate-50 pb-3 mb-4">
                      ...
                      <div className="pr-12">
                        ...
                        <p className="text-[11px] text-emerald-600 uppercase tracking-tighter font-semibold">Ficha com {batch.length} itens</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-[12px] font-semibold text-slate-600 uppercase truncate">{staff?.name || 'Desconhecido'}</p>
                      </div>
                      <div className="space-y-1 text-right">
                        <div className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-400 uppercase tracking-wider justify-end">
                          <Clock className="w-2.5 h-2.5" /> Horário
                  ...
                  <button onClick={() => setViewingBatch(batch)} className="w-full py-4 bg-emerald-50 text-emerald-600 flex items-center justify-center gap-3 text-[12px] font-semibold uppercase tracking-widest border-t border-emerald-100 active:bg-emerald-100 transition-all">
