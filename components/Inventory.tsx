    ...
    <div className="space-y-6 sm:space-y-10">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={`relative bg-white w-full max-w-sm p-8 shadow-2xl border-t-8 ${theme.border} animate-in zoom-in duration-200`}>
            ...
            <div className="text-center mb-8">
              ...
              <h3 className="text-lg font-black text-slate-800 uppercase tracking-tighter mb-1">Ajustar Inventário</h3>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Novo Saldo em Estoque</label>
                <div className="relative">
                  <input 
                    ...
                    autoFocus
                    className={`w-full bg-slate-50 border border-slate-200 px-4 py-4 text-center text-2xl font-black tracking-tighter outline-none ${theme.primaryFocus} transition-all`}
                  />
              ...
              <div className="grid grid-cols-2 gap-3">
                <button 
                  onClick={() => setEditingProduct(null)}
                  disabled={isSaving}
                  className="py-3 bg-slate-100 text-slate-600 text-xs font-bold uppercase tracking-widest hover:bg-slate-200 transition-colors disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button 
                  ...
                  disabled={isSaving}
                  className={`py-3 ${theme.accent} text-white text-xs font-bold uppercase tracking-widest hover:opacity-90 transition-all flex items-center justify-center gap-2 disabled:opacity-50`}
                >
      ...
      <header className="border-b border-slate-200 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-[10px] sm:text-xs text-slate-500 mt-1 uppercase tracking-[0.2em] font-medium">Controle de Saldo e Disponibilidade</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <div className="relative">
            ...
            <select 
              ...
              className={`pl-9 pr-8 py-2.5 bg-white border border-slate-200 text-xs uppercase font-bold tracking-[0.2em] outline-none ${theme.primaryFocus} w-full appearance-none shadow-sm`}
            >
              {categories.map(cat => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
        {filteredProducts.length > 0 ? (
          filteredProducts.map((p) => (
            <div key={p.id} className="bg-white border border-slate-200 p-5 shadow-sm relative group overflow-hidden transition-all hover:border-slate-300">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-bold uppercase tracking-widest bg-slate-100 text-slate-400 px-2 py-0.5">
                  {p.category}
                </span>
                {p.stock <= 5 && (
                  <div className="flex items-center gap-1 text-red-500 animate-pulse">
                    <AlertCircle className="w-3 h-3" />
                    <span className="text-[10px] font-bold uppercase tracking-tighter">Baixo Estoque</span>
                  </div>
                )}
              </div>
              
              <h3 className="text-xs font-bold text-slate-800 uppercase leading-tight mb-6 h-8 line-clamp-2">
                {p.name}
              </h3>
          ...
          <div className="col-span-full py-20 bg-white border border-slate-200 border-dashed flex flex-col items-center justify-center text-center">
            ...
            <p className="text-xs uppercase font-bold tracking-[0.3em] text-slate-300">Nenhum item localizado com esses filtros</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-10 p-6 bg-slate-50 border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Estoque Regular</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-red-500"></div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Estoque Crítico (≤ 5)</span>
          </div>
        </div>
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Total de Itens: {filteredProducts.length}</p>
      </div>
    </div>
  );
};
